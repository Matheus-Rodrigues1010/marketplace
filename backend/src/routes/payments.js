...
// Rota de criação de conta (sem alterações, mas incluída para o arquivo completo)
router.post('/create-connected-account', auth, async (req, res) => {
  ...
  try {
    ...
    if (userResult.rows[0].stripe_account_id) { return res.status(400).json({ error: 'Este usuário já é um vendedor.' }); }
    const account = await stripe.accounts.create({
      type: 'express', email: userResult.rows[0].email,
...
// --- NOVA ROTA: Criar um link para o dashboard do vendedor ---
...
router.get('/seller-dashboard-link', auth, async (req, res) => {
  ...
  try {
    ...
    // Gera um link de login único para o dashboard da conta conectada
